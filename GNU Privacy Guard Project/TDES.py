    ...
    def decrypt(self, data):
        ...
        if self.mode == 'ECB':
            ...
            if not isinstance(data, (bytes, bytearray)):
                raise TypeError("GPG mode expects bytes input")
            if len(data) < 16:
                raise ValueError("Ciphertext is not long enough for GPG")

            def _3DES_enc_block(block_bytes: bytes) -> bytes:
                """Encrypt a single 8-byte block under 3DES E_k1 D_k2 E_k3 (matches encrypt() behavior)."""
                if len(block_bytes) != 8:
                    block_bytes = block_bytes.ljust(8, b'\x00')
                bits = coreDES.byte2bitArray(block_bytes)
                bits = coreDES.encryptBlock(bits, self.subkey1)
                bits = coreDES.encryptBlock(bits, self.subkey2)
                bits = coreDES.encryptBlock(bits, self.subkey3)
                return coreDES.bitArray2byte(bits)

            def _xor_bytes(a: bytes, b: bytes) -> bytes:
                return bytes(x ^ y for x, y in zip(a, b))

            # split first two 8-byte blocks (C0, C1) and remaining ciphertext
            C0 = bytes(data[0:8])
            C1 = bytes(data[8:16])
            rest = bytes(data[16:])

            # FRE = CIPH(IV) where IV = all-zero block
            FRE = _3DES_enc_block(b'\x00' * 8)
            P0 = _xor_bytes(FRE, C0)

            # FRE2 = CIPH(C0)
            FRE2 = _3DES_enc_block(C0)
            P1 = _xor_bytes(FRE2, C1)

            # Quick-check: last two bytes of P0 must equal first two bytes of P1
            if P0[6:8] != P1[0:2]:
                raise ValueError("Invalid decryption ~ bad key or corrupted data")

            # Collect plaintext: prefix (P0||P1) plus decrypted rest
            plaintext = P0 + P1
            prevC = C1

            # Decrypt remaining blocks (use prevC as feedback)
            for i in range(0, len(rest), 8):
                Ci = rest[i:i + 8]
                # FREn = CIPH(prevC)
                FREn = _3DES_enc_block(prevC)
                if len(Ci) < 8:
                    # partial final block: XOR only the available bytes
                    xored = _xor_bytes(FREn[:len(Ci)], Ci)
                    plaintext += xored
                    prevC = Ci  # not further used
                else:
                    Pi = _xor_bytes(FREn, Ci)
                    plaintext += Pi
                    prevC = Ci
            return plaintext[10:]
